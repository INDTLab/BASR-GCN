...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 70, 'input_size': (3, 224, 224), 'pool_size': None,
...
class Stem(nn.Module):
    ...
    """
    def __init__(self, img_size=224, in_dim=5, out_dim=768, act='relu'):
        super().__init__()        
        self.convs = nn.Sequential(
            nn.Conv1d(in_dim, out_dim//2, 3, stride=2, padding=1),
            nn.BatchNorm1d(out_dim//2),
            act_layer(act),
            nn.Conv1d(out_dim//2, out_dim, 3, stride=2, padding=1),
...
class Downsample(nn.Module):
    ...
    """
    def __init__(self, in_dim=4, out_dim=768):
        super().__init__()        
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, channels[0], num_nodes//4))
        HW = num_nodes
        ...
        self.prediction = Seq(nn.Conv1d(channels[-1], 1024, 1, bias=True),
                              nn.BatchNorm1d(1024),
                              act_layer(act),
...
def pvig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=70, drop_path_rate=0.0, **kwargs):
...
def pvig_s_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=22, drop_path_rate=0.0, **kwargs):
            self.k = 6 # neighbor num (default:9)
            ...
            self.use_dilation = True # use dilated knn or not
            self.epsilon = 0.0 # stochastic epsilon for gcn
            self.use_stochastic = True # stochastic for gcn, True or False
            self.drop_path = drop_path_rate
            self.blocks = [2,2,6,2] # number of basic blocks in the backbone
